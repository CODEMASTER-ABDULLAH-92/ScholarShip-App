...
const logoutRecuriter = async () => {
  ...
  const stats = [
    ...
    { title: "Approval Rate", value: "68%", icon: <CheckCircle size={20} className="text-yellow-600" />, change: "2% better" },
  ];
  ...
  const recentApplications = [
    { id: 1, name: "Sarah Johnson", scholarship: "Merit Scholarship", status: "pending", date: "2 hours ago" },
    { id: 2, name: "Michael Chen", scholarship: "Need-Based Aid", status: "approved", date: "5 hours ago" },
    { id: 3, name: "Emma Williams", scholarship: "Merit Scholarship", status: "rejected", date: "1 day ago" },
    { id: 4, name: "David Kim", scholarship: "Need-Based Aid", status: "pending", date: "1 day ago" },
  ];

  const getStatusBadge = (status) => {
    switch(status) {
      case 'active': return <span className="px-2 py-1 text-xs font-medium rounded-full bg-green-100 text-green-800">Active</span>;
      case 'draft': return <span className="px-2 py-1 text-xs font-medium rounded-full bg-gray-100 text-gray-800">Draft</span>;
      case 'ended': return <span className="px-2 py-1 text-xs font-medium rounded-full bg-red-100 text-red-800">Ended</span>;
      case 'pending': return <span className="px-2 py-1 text-xs font-medium rounded-full bg-yellow-100 text-yellow-800">Pending</span>;
      case 'approved': return <span className="px-2 py-1 text-xs font-medium rounded-full bg-blue-100 text-blue-800">Approved</span>;
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        ...
        <div className="mb-8">
          <div className="flex flex-wrap gap-3">
            <Link to={"/dashboard/add-data"} className="flex items-center gap-2 bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg text-sm font-medium">
              <Plus size={16} />
                ...
                {data.map((scholarship) => (
                  <tr key={scholarship.id}>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                      <Link to={`/dashboard/update-scholarship/${scholarship._id}`} className="text-blue-600 hover:text-blue-900 mr-3">Edit</Link>
                      <button onClick={() => handleDelete(scholarship._id)} className="text-red-600 ">Delete</button>
                    </td>
                  </tr>
                ))}
              </tbody>
