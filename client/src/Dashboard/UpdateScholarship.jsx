...
import { 
  ...
  DollarSign, ClipboardList, CheckCircle, 
...
const UpdateScholarship = () => {
  ...
  useEffect(() => {
    if (data && data.length > 0) {
      const scholarship = data.find((scholarship) => scholarship._id === id);
      if (scholarship) {
        setFormDataState({
          ...
          gpaRequirement: scholarship.gpaRequirement || 3.7,
          ...
          benefitRenewable: scholarship.benefitRenewable || false,
          additionalBenefits: scholarship.additionalBenefits || "",
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.put(
        `${url}/api/scholarship/update-scholarShipDetails/${id}`,
        submissionData,
        {
          headers: {
      ...
      if (response.data.success) {
        ...
        toast.error(response.data.message || "Failed to update scholarship");
      }
    } catch (error) {
      console.error("Error in updating scholarship details", error);
      toast.error(
